...
import com.tinet.ctilink.entity.LogWeb400Call;
import com.tinet.ctilink.inc.Const;
import com.tinet.ctilink.inc.Macro;
import com.tinet.ctilink.model.*;
...
public class WebCallExecutor {
	...
	public WebCallResponse execute(WebCall webCall) {
		...
		if (sendTts(webCall)) {
			...
			if (webCall.getTel().contains(",")) { // 多号码呼叫
				...
				if (telList.length > 0) {
					for (int i = 0; i < telList.length; ++i) {
						...
						try {
							...
							webCall.setResult(Integer.valueOf(executeCall(webCall.getTel(), webCall.getSubtel(), webCall.getEnterpriseId().intValue(),
									webCall.getUniqueId(), webCall.getParams(), webCall.getIvrId(),
									webCall.getTimeout().intValue(), webCall.getClidNumber(), webCall.getAmd(),
									web400Push, sb)));
							webCall.setCdrUniqueId(sb.toString());
							if (webCall.getResult().intValue() == 1) { // 呼叫成功了返回answerTime
				...
				try {
					...
					StringBuilder sb = new StringBuilder();
					webCall.setResult(
							Integer.valueOf(executeCall(webCall.getTel(),
									webCall.getSubtel(), webCall.getEnterpriseId().intValue(), webCall.getUniqueId(),
									webCall.getParams(), webCall.getIvrId(), webCall.getTimeout().intValue(),
		...
		String webCallResultMessage = null;
		...
		if (StringUtils.isNotEmpty(webCall.getCallbackUrl())) { // 回调地址不是空
			// 获取curl级别
			int level = 0;
			EnterpriseSetting setting = redisService.get(String.format(CacheKey.ENTERPRISE_SETTING_ENTERPRISE_ID_NAME,
	...
	public int executeCall(String tel, String subtel, int enterpriseId, String uniqueid, Map<String, String> params, String ivrId, int timeout, String clidNumber, Integer amd, boolean web400Push, StringBuilder cdrUniqueId) {
		try {
			...
			if (res == Const.WEB400_CALL_CHECK_OK) {
				...
				EnterpriseSetting enterpriseSetting = redisService.get(String.format(CacheKey.ENTERPRISE_SETTING_ENTERPRISE_ID_NAME,
	...
	private int check(String tel, String ext, int enterpriseId, String ivrId, String clidNumber) {
		...
		// check enterpriseId
		Entity entity = redisService.get(String.format(CacheKey.ENTITY_ENTERPRISE_ID, enterpriseId), Entity.class);
		if (entity == null) {
			return Const.WEB400_CALL_ILLEGAL_ENTERPRISE;
		}
		// get a trunk number from table trunk
		List<EnterpriseHotline> enterpriseHotlines = redisService.getList(String.format(CacheKey.ENTERPRISE_HOTLINE_ENTERPRISE_ID,
		...
		// timeout unit: millisecond
		if (Macro.AMI_RESPONSE_UNIT.equals(Const.SYSTEM_SETTING_PROPERTY_SECOND)) {
			amiResponseTimeout *= 1000;
		} else if (Macro.AMI_RESPONSE_UNIT.equals(Const.SYSTEM_SETTING_PROPERTY_MINUTE)) {
			amiResponseTimeout *= 1000 * 60;
		} else if (Macro.AMI_RESPONSE_UNIT.equals(Const.SYSTEM_SETTING_PROPERTY_HOUR)) {
		...
		if (null != enterpriseSetting) {
			...
			if (!"".equals(value) && null != value) {
				...
				RestrictTel restrictTel = redisService.get(String.format(CacheKey.RESTRICT_TEL_ENTERPRISE_ID_TYPE_RESTRICT_TYPE_TEL,
		...
		List<EnterpriseHotline> enterpriseHotlineList = redisService.getList(String.format(CacheKey.ENTERPRISE_HOTLINE_ENTERPRISE_ID, enterpriseId),
		...
		if (StringUtils.isNotEmpty(clidNumber)) {
			EnterpriseHotline enterpriseHotline = redisService.get(String.format(CacheKey.ENTERPRISE_HOTLINE_ENTERPRISE_ID_NUMBER_TRUNK, enterpriseId, clidNumber),
					EnterpriseHotline.class);
			// check clid
			if (enterpriseHotline != null) {
				clid = clidNumber;
			} else {
				EnterpriseClid enterpriseClid = redisService.get(String.format(CacheKey.ENTERPRISE_CLID_ENTERPRISE_ID, enterpriseId), EnterpriseClid.class);
		...
		// defined template.
		IvrProfile ivrProfile = redisService.get(String.format(CacheKey.IVR_PROFILE_ENTERPRISE_ID_ID, enterpriseId, web400IvrId), IvrProfile.class);
		if (ivrProfile == null) {
