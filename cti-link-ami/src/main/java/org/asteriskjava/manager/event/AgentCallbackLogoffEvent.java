...
/**
 * An AgentCallbackLogoffEvent is triggered when an agent that previously logged
 * in using AgentCallbackLogin is logged of.
 * <p>
 * It is implemented in <code>channels/chan_agent.c</code>
 * 
 * @see AgentCallbackLoginEvent
 * @author srt
...
public class AgentCallbackLogoffEvent extends ManagerEvent {
	...
	/**
	 * Returns the reason for the logoff. The reason is set to Autologoff if the
	 * agent has been logged off due to not answering the phone in time.
	 * Autologoff is configured by setting <code>autologoff</code> to the
	 * appropriate number of seconds in <code>agents.conf</code>.
	 */
