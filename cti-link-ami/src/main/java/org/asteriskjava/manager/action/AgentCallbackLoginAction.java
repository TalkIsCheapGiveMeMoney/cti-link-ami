 ...
 * in an agent.
 * <p>
 * Available since Asterisk 1.2, deprecated in Asterisk 1.4 and removed in
 * Asterisk 1.6.
 *
 * @author srt
 * @version $Id: AgentCallbackLoginAction.java 1146 2008-08-20 17:04:18Z srt $
 * @since 0.2
 * @deprecated use {@link QueueAddAction}
 *             instead.
 */
public class AgentCallbackLoginAction extends AbstractManagerAction {
	 ...
	 *            the context of the extension to use for callback
	 * @param ackCall
	 *            <code>Boolean.TRUE</code> to require an acknowledgement by '#'
	 *            when agent is called back, <code>Boolean.FALSE</code>
	 *            otherwise. <code>null</code> if default should be used.
	 * @param wrapupTime
	 *            the minimum amount of time (in seconds) after disconnecting
	 *            before the caller can receive a new call. <code>null</code> if
	 ...
	 *
	 * @return <code>Boolean.TRUE</code> if acknowledgement by '#' is required
