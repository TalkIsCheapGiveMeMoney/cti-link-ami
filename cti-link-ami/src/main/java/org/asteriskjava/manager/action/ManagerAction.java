...
public interface ManagerAction extends Serializable {
	 ...
	 * returned by the Asterisk server will include the same id. This way the
	 * action id can be used to track actions and their corresponding responses
	 * and response events.
	 * <p>
	 * Note that Asterisk-Java uses its own internal action id to match actions
	 * with the corresponding responses and events. Though the internal action
	 * is never exposed to the application code. So if you want to handle
	 * reponses or response events on your own your application must set a
	 * unique action id using this method otherwise the action id of the reponse
	 * and response event objects passed to your application will be null.
	 * 
	 * @param actionId
	 *            the user provided action id to set.
