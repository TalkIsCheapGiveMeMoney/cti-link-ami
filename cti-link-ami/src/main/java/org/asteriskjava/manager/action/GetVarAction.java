 ...
 * <code>CALLERIDNUM</code> using the Manager API. You can only read channel
 * variables that you have explicitly set using the <code>Set</code> or
 * <code>SetVar</code> (for Asterisk 1.0.x) applications in the dialplan, in an
 * AGI script or by using the
 * {@link org.asteriskjava.manager.action.SetVarAction} through the Manager API
 * itself.
 * <p>
 * GetVarAction returns a
 * {@link org.asteriskjava.manager.response.GetVarResponse}. To get the actual
 ...
 * with either the variable name as parameter (for Asterisk 1.0.x) or with
 * "Value" as parameter (for Asterisk since 1.2).
 * <p>
 ...
 * {@link org.asteriskjava.manager.response.GetVarResponse#getValue()} when
 * using Asterisk 1.2 or later.
 * <p>
