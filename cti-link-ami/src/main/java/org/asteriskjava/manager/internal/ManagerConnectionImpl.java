...
public class ManagerConnectionImpl implements ManagerConnection, Dispatcher {
	 ...
	 * ManagerAction.
	 */
	private long defaultResponseTimeout = 5000;

	/**
	 * The default timeout to wait for the last ResponseEvent after sending an
	 * EventGeneratingAction.
	 */
	private long defaultEventTimeout = 10000;
	...
	public synchronized void login(String eventMask) throws IOException, AuthenticationFailedException, TimeoutException {
		if (state != INITIAL && state != DISCONNECTED) {
			throw new IllegalStateException(
					"Login may only be perfomed when in state " + "INITIAL or DISCONNECTED, but connection is in state " + state);
		}
	...
	protected synchronized void doLogin(long timeout, String eventMask) throws IOException, AuthenticationFailedException, TimeoutException {
		...
		if (challengeResponse instanceof ChallengeResponse) {
			...
			throw new AuthenticationFailedException(
					"Unable to get challenge from Asterisk. ChallengeAction returned: " + challengeResponse
							.getMessage());
		}
	...
	public synchronized void logoff() throws IllegalStateException {
		if (state != CONNECTED && state != RECONNECTING) {
			throw new IllegalStateException(
					"Logoff may only be perfomed when in state " + "CONNECTED or RECONNECTING, but connection is in state " + state);
		}
	...
	public ManagerResponse sendAction(ManagerAction action, long timeout) throws IOException, TimeoutException, IllegalArgumentException, IllegalStateException {
		...
		// still no response?
		if (result.getResponse() == null) {
			throw new TimeoutException("Timeout waiting for response to " + action
					.getAction() + (action.getActionId() == null ? "" : " (actionId: " + action.getActionId() + ")"));
		}
	...
	public ResponseEvents sendEventGeneratingAction(EventGeneratingAction action, long timeout) throws IOException, EventTimeoutException, IllegalArgumentException, IllegalStateException {
		...
		if (action == null) {
			...
			throw new IllegalArgumentException(
					"Unable to send action: actionCompleteEventClass (" + action.getActionCompleteEventClass()
							.getName() + ") for " + action.getClass().getName() + " is not a ResponseEvent.");
		}
		...
		if ((responseEvents.getResponse() == null || !responseEvents.isComplete())) {
			...
			throw new EventTimeoutException("Timeout waiting for response or response events to " + action
					.getAction() + (action.getActionId() == null ? "" : " (actionId: " + action.getActionId() + ")"),
					responseEvents);
		}
		...
		if (internalActionId != null) {
			synchronized (this.responseListeners) {
				...
				if (listener != null) {
					...
					logger.debug(
							"No response listener registered for " + "internalActionId '" + internalActionId + "'");
				}
			}
		} else {
			logger.error(
	...
	public void dispatchEvent(ManagerEvent event) {
				...
				synchronized (responseEventListeners) {
					...
					if (listener != null) {
						try {
							...
							logger.warn(
									"Unexpected exception in response event listener " + listener.getClass().getName(),
									e);
						}
					}
				}
		...
		if (event instanceof DisconnectEvent) {
			...
			if (state == CONNECTED) {
				// 先放fireEvent保证在reconnect时login中initialized是false。否则有可能login成了做initialize()时发现initialized还是true不会做初始化
				fireEvent(event);
				state = RECONNECTING;
				...
				reconnectThread.setName("Asterisk-Java ManagerConnection-" + id + "-Reconnect-" + reconnectThreadCounter
						.getAndIncrement());
	...
	private void setProtocolIdentifier(final String identifier) {
		...
		&& !"OpenPBX Call Manager/1.0".equals(identifier) && !"CallWeaver Call Manager/1.0"
				.equals(identifier) && !(identifier != null && identifier.startsWith("Asterisk Call Manager Proxy/"))) {
