...
class AsteriskChannelImpl extends AbstractLiveObject implements AsteriskChannel {
	 ...
	 * @param name
	 *            name of this channel, for example "SIP/1310-20da".
	 * @param id
	 ...
	 */
	AsteriskChannelImpl(final AsteriskServerImpl server, final String name, final String id, final Date dateOfCreation) throws IllegalArgumentException {
		super(server);
	 ...
	 * @param id
	 *            the new unique id of this channel.
	 */
	...
	public boolean wasBusy() {
		return wasInState(
				ChannelState.BUSY) || hangupCause == HangupCause.AST_CAUSE_BUSY || hangupCause == HangupCause.AST_CAUSE_USER_BUSY;
	}
	 ...
	 * @param extension
	 *            the visted dialplan entry to add.
	 */
	 ...
	 *
	 * @param dateOfRemoval
	 *            date the channel was hung up
	 * @param hangupCause
	 *            cause for hangup
	 * @param hangupCauseText
	 *            textual representation of hangup cause
	 */
	 ...
	 */
	public String getNoCacheVariable(String variable) throws ManagerCommunicationException, NoSuchChannelException {
		ManagerResponse response;
		String value;
	...
	public void startMonitoring(String filename, String format, boolean mix) throws ManagerCommunicationException, NoSuchChannelException {
		startMonitoring(filename, format, mix, "all");// all in out
	}

	public void startMonitoring(String filename, String format, boolean mix, String direction) throws ManagerCommunicationException, NoSuchChannelException {
		ManagerResponse response;

		response = server.sendAction(new MonitorAction(name, filename, format, mix, direction));
		if (response instanceof ManagerError) {
			throw new NoSuchChannelException("Channel '" + name + "' is not available: " + response.getMessage());
		}
	}
