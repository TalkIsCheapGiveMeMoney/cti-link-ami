...
class AsteriskQueueImpl extends AbstractLiveObject implements AsteriskQueue {
	...
	AsteriskQueueImpl(AsteriskServerImpl server, String name, Integer max, String strategy, Integer serviceLevel, Integer weight) {
		...
		/*
		 * 删除这段代码，不使用asterisk-java里面的服务水平功能，by 安静波 20130725 timer = new
		 * Timer("ServiceLevelTimer-" + name, true);
		 */
	 ...
	 *
	 * @param channel
	 *            the channel that joined the queue
	 * @param reportedPosition
	 *            the position as given by Asterisk (currently not used)
	 * @param dateReceived
	 *            the date the hannel joined the queue
	 */
	void createNewEntry(AsteriskChannelImpl channel, int reportedPosition, Date dateReceived) {
		...
		/*
		 * 删除这段代码，不使用asterisk-java里面的服务水平功能，by 安静波 20130725 long delay =
		 * serviceLevel * 1000L; if (delay > 0) { ServiceLevelTimerTask
		 * timerTask = new ServiceLevelTimerTask(qe); timer.schedule(timerTask,
		 * delay); synchronized (serviceLevelTimerTasks) {
	 ...
	 *
	 * @param entry
	 *            an existing entry object.
	 * @param dateReceived
	 *            the remove event was received.
	 */
	void removeEntry(AsteriskQueueEntryImpl entry, Date dateReceived) {
		/*
		 * 删除这段代码，不使用asterisk-java里面的服务水平功能，by 安静波 20130725 synchronized
		 * (serviceLevelTimerTasks) { if
		 * (serviceLevelTimerTasks.containsKey(entry)) { ServiceLevelTimerTask
		 * timerTask = serviceLevelTimerTasks.get(entry); timerTask.cancel();
