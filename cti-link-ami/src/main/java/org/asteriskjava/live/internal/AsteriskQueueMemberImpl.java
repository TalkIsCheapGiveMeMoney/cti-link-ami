...
class AsteriskQueueMemberImpl extends AbstractLiveObject implements AsteriskQueueMember {
	 ...
	 * @param queue
	 *            queue this member is registered to.
	 * @param location
	 *            location of member.
	 * @param state
	 *            state of this member.
	 * @param paused
	 *            <code>true</code> if this member is currently paused,
	 *            <code>false</code> otherwise.
	 * @param penalty
	 *            penalty of this member.
	 * @param membership
	 *            "dynamic" if the added member is a dynamic queue member,
	 *            "static" if the added member is a static queue member.
	 */
	AsteriskQueueMemberImpl(final AsteriskServerImpl server, final AsteriskQueueImpl queue, String location, QueueMemberState state, boolean paused, Integer penalty, String membership) {
		super(server);
		this.queue = queue;
	...
	private void sendPauseAction(QueuePauseAction action) throws ManagerCommunicationException, NoSuchInterfaceException {
		...
		if (response instanceof ManagerError) {
			...
			if (action.getQueue() != null) {
				// Message: Interface not found
				throw new NoSuchInterfaceException("Unable to change paused state for '" + action
						.getInterface() + "' on '" + action.getQueue() + "': " + response.getMessage());
			} else {
				throw new NoSuchInterfaceException("Unable to change paused state for '" + action
						.getInterface() + "' on all queues: " + response.getMessage());
			}
		}
	}
	...
	public void setPenalty(int penalty) throws IllegalArgumentException, ManagerCommunicationException, InvalidPenaltyException {
		...
		if (response instanceof ManagerError) {
			throw new InvalidPenaltyException("Unable to set penalty for '" + location + "' on '" + queue
					.getName() + "': " + response.getMessage());
		}
	}
